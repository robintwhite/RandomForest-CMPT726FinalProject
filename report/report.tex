...
Regression splitting using variance. TODO some explanation and references here. It is noted the regressor implementation has a significantly
higher mean square error compared to Weka and scikit-learn. This could be attributed to the greater variation in the tree depths and node sizes
observed in the other two implementations. For example, scikit-learn's optimal random forest regressor contains trees varying in depth from 5 to 16 and
varing in minimum node size from 1 to 4. 
...
Using optimal values for learning shown in ~\ref{opt-table}, time for 4 processes of 20seconds is a significant improvement over a single process taking 1min 4sec for training GP\_greater\_than\_0. After tarining however, prediction time is considerably lower, taking $\approx$1sec regardless.
...
For comparison, Decision tree classifier and Support Vector Machine (SVM) were fitted and tested on the data. The test set accuracy of the decision tree was 64\% while the test accuracy
of the SVM was 53\%. 
...
Also, the running time can be decreased by optimizing split function subroutines and the data structures to use Cython or C. Once the running time is improved,
it will also be feasible to increase tree depth and attempt to fit a better regression model on the data.
